...
* L3 — Event roles & overrides

Not every calendar entry blocks *my* time. Each event has a role:
- =busy= — consumes my time → removed from free slots;
- =info= — time-bound context (a colleague's training, custody week, school) →
  visible, but does NOT block;
- =ignore= — not shown at all (e.g. week numbers).

Role is decided by: a per-event **override** (highest, kept on the Emacs side
since source calendars are read-only) → per-calendar policy (=ignore=/=info=
lists) → the event's Apple **availability** (Free ⇒ info) → default =busy=.
The override is the safety net when Apple's classification is wrong.
...
* L3 — Per-user calendar classification

The per-calendar busy/info/ignore policy is **personal data**, not package
config: it is kept in `org-apple-calendar-classification-file` (point it at your
private, encrypted data area). On first calendar access the file is **created
listing all your calendars** (default role `busy`); new calendars are added
automatically over time. You edit the roles; existing roles are never clobbered.
This keeps real calendar names out of any shared config.
