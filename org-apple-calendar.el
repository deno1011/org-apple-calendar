...
(defun org-apple-calendar-event-by-uid (uid &optional mirror-file)
  "Return the mirrored Apple calendar event with UID.
MIRROR-FILE defaults to `org-apple-calendar-mirror-file'.  This reads the
generated mirror as input and does not contact EventKit."
  (unless (and uid (stringp uid) (not (string-empty-p uid)))
    (user-error "Event UID must be a non-empty string"))
  (let ((file (or mirror-file org-apple-calendar-mirror-file)))
    (unless (and file (file-readable-p file))
      (user-error "Calendar mirror is not readable: %s" file))
    (with-temp-buffer
      (insert-file-contents file)
      (org-mode)
      (goto-char (point-min))
      (let ((pattern (format "^[ \t]*:APPLE_EVENT_UID:[ \t]*%s[ \t]*$"
                             (regexp-quote uid))))
        (or (catch 'found
              (while (re-search-forward pattern nil t)
                (let ((next-pos (match-end 0))
                      event)
                  (org-back-to-heading t)
                  (setq event (org-apple-calendar--event-at-point))
                  (when (and event (equal (plist-get event :uid) uid))
                    (throw 'found event))
                  (goto-char next-pos)))
              nil)
            (user-error "No event with UID in calendar mirror: %s" uid))))))
...
(defun org-apple-calendar--append-adopted-appointment (ev apple-uid)
  ...
  (let ((file org-apple-calendar-source-file)
    ...
    (with-current-buffer (find-file-noselect file)
      ...
      (save-buffer)
      (list :file file
            :title (or (plist-get ev :title) "(Termin)")
            :adopted-from-calendar (or (plist-get ev :calendar) "")
            :adopted-from-uid (or (plist-get ev :uid) "")
            :adopted-from-key key
            :apple-event-id apple-uid
            :apple-calendar org-apple-calendar-target-calendar))))

(defun org-apple-calendar-adopt-event (ev &optional confirm)
  "Adopt Apple calendar event EV into `org-apple-calendar-source-file'.
When CONFIRM is non-nil, ask before creating the target Apple event.  Return a
plist describing the adopted event, source override, and target Apple event."
  (unless (eq org-apple-calendar-write-backend 'eventkit)
    (user-error "Adopt requires the EventKit write backend"))
  (let* ((source-uid (and ev (plist-get ev :uid)))
         (key (and ev (org-apple-calendar--adopt-key ev))))
    (unless ev
      (user-error "No Apple calendar event"))
    (unless source-uid
      (user-error "Event has no Apple UID"))
    ...
    (let ((res (org-apple-calendar--eventkit-create-event
      ...
      (let* ((heading (org-apple-calendar--append-adopted-appointment
                       ev (plist-get res :uid)))
             (role (org-apple-calendar-set-event-role source-uid 'ignore)))
        (ignore-errors (org-apple-calendar-refresh-mirror))
        (message "Adopted '%s' into %s and ignored source event"
                 (or (plist-get ev :title) "(Termin)")
                 org-apple-calendar-target-calendar)
        (list :kind 'org-apple-calendar-adoption
              :title (or (plist-get ev :title) "(Termin)")
              :source-calendar (or (plist-get ev :calendar) "")
              :source-uid source-uid
              :source-key key
              :source-ignored (eq role 'ignore)
              :target-calendar org-apple-calendar-target-calendar
              :target-apple-event-id (plist-get res :uid)
              :target-file org-apple-calendar-source-file
              :heading heading
              :override (list :file org-apple-calendar-overrides-file
                              :event-uid source-uid
                              :role role
                              :changed t))))))

(defun org-apple-calendar-adopt-event-by-uid (uid &optional mirror-file)
  "Adopt mirrored Apple calendar event UID through the package-owned flow.
MIRROR-FILE defaults to `org-apple-calendar-mirror-file'.  This is the
non-interactive API for external runtimes such as `emacs-agent-runtime'."
  (org-apple-calendar-adopt-event
   (org-apple-calendar-event-by-uid uid mirror-file)))
...
and therefore hides all occurrences represented by that UID."
  ...
  (plist-get
   (org-apple-calendar-adopt-event
    (org-apple-calendar--event-at-point)
    (called-interactively-p 'any))
   :target-apple-event-id))
